...
dirParent=getDirectory("Choose Data Folder");
subfolderList=recursiveSubFolderIndexing(dirParent);

// Keep only raw puncta files folders from the sub-folder list //
subfolderListTemp=newArray();
for (folderCount=0; folderCount<subfolderList.length; folderCount++) {
	if (endsWith(subfolderList[folderCount], "RawPunctaFiles/")){
		subfolderListTemp=Array.concat(subfolderListTemp,subfolderList[folderCount]);
	}
}

subfolderList=subfolderListTemp;

// get default speed limit and time step gap for the folder //
Dialog.create("Parameter");
Dialog.addNumber("Enter Default Speed Limit (um/s)", 0.15);
Dialog.addNumber("Enter time step gap (s)", 5);
Dialog.show();

defaultSpeedLimit=Dialog.getNumber();
stepGapInSeconds=Dialog.getNumber();



////Entereing loop for all the files /////
for(subfolderCount=0; subfolderCount<subfolderList.length;subfolderCount++){
	dir=subfolderList[subfolderCount];
	list=getFileList(dir);
	parentDirectory=File.getParent(dir);
	imageName=File.getName(parentDirectory);
...
function parameterinput()   
  {
  directoryHavingKymoFile=File.getParent(dir);
			...
			for (FileNumber=0; FileNumber<fileList.length; FileNumber++) {	
				if (endsWith(fileList[FileNumber], "_kymo.tif")==1){
					open(directoryHavingKymoFile+File.separator+fileList[FileNumber]);
				kymoImageFound=1;
				}
			}
			if (kymoImageFound==1){
   ...
   Dialog.addNumber("Speed Limit Low(um/s):", defaultSpeedLimit);
